...
const Footer = () => {
  ...
  const socialLinks = [
    { icon: <Facebook className="w-5 h-5" />, label: 'Facebook', href: 'https://facebook.com/backbenchercoder', color: 'hover:bg-blue-600' },
    { icon: <Twitter className="w-5 h-5" />, label: 'Twitter', href: 'https://twitter.com/backbenchercoder', color: 'hover:bg-sky-500' },
    { icon: <Instagram className="w-5 h-5" />, label: 'Instagram', href: 'https://instagram.com/backbenchercoder', color: 'hover:bg-pink-600' },
    { icon: <Linkedin className="w-5 h-5" />, label: 'LinkedIn', href: 'https://linkedin.com/company/backbenchercoder', color: 'hover:bg-blue-700' },
    { icon: <Github className="w-5 h-5" />, label: 'GitHub', href: 'https://github.com/backbenchercoder', color: 'hover:bg-gray-800' },
    { icon: <Youtube className="w-5 h-5" />, label: 'YouTube', href: 'https://youtube.com/@backbenchercoder', color: 'hover:bg-red-600' },
  ];
  ...
  const features = [
    ...
    { icon: <CreditCard className="w-5 h-5" />, text: 'Secure Payment', color: 'text-blue-400' },
    { icon: <Headphones className="w-5 h-5" />, text: '24/7 Support', color: 'text-purple-400' },
  ];

  // Payment methods with icons
  const paymentMethods = [
    { name: 'VisaCard', icon: '💳', bgColor: 'bg-blue-500/20', textColor: 'text-blue-400' },
    { name: 'Pubali Bank', icon: '🏦', bgColor: 'bg-green-500/20', textColor: 'text-green-400' },
    { name: 'Binance', icon: '₿', bgColor: 'bg-yellow-500/20', textColor: 'text-yellow-400' },
  ];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-2xl font-bold text-white">Backbencher Coder</h3>
                <p className="text-gray-400 text-sm">Transforming Ideas into Reality</p>
              </div>
            </div>
            
            <p className="text-gray-300 leading-relaxed text-sm md:text-base">
              We create exceptional digital experiences that drive growth and innovation for businesses worldwide.
            </p>
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 text-gray-300 hover:text-white transition-colors">
                ...
                <span className="text-sm md:text-base break-all">backbenchercoder.official@gmail.com</span>
              </div>
              <div className="flex items-center gap-3 text-gray-300 hover:text-white transition-colors">
                <MapPin className="w-5 h-5 text-[#D9FDA3]" />
                <span className="text-sm md:text-base">Noumohol, Mymensingh, Bangladesh</span>
              </div>
            </div>
          </div>
                  ...
                  {section.links.map((link, linkIndex) => (
                    <li key={linkIndex}>
                      ...
                      >
                        ...
                        <span className="text-sm md:text-base truncate">{link.name}</span>
                      </Link>
                    </li>
                  ))}
                </ul>
              ...
              <div className="flex flex-wrap gap-3">
                {paymentMethods.map((method, index) => (
                  <div
                    ...
                    title={method.name}
                  >
                    <span className={`text-lg ${method.textColor}`}>{method.icon}</span>
